...
class VisualOdometry(object):
    def __init__(self, cam, groundtruth, feature_tracker : FeatureTracker):
        ...
        # self.matched_ref_features = None
        # self.matched_cur_features = None
    ...
    def processFrame(self, frame_id, matched_ref_features, matched_cur_features, matched_cur_descriptors):
        # track features 
        ...
        R, t = self.estimatePose(matched_ref_features, matched_cur_features)     
        self.timer_pose_est.refresh()
        # update keypoints history  
        self.kps_ref = matched_ref_features
        self.kps_cur = matched_cur_features
        self.des_cur = matched_cur_descriptors
        self.num_matched_kps = self.kpn_ref.shape[0] 
        self.num_inliers =  np.sum(self.mask_match)
        #compute average delta pixel shift
        self.average_pixel_shift = np.mean(np.abs(matched_ref_features - matched_cur_features))
    ...
    def track(self, img, frame_id, matched_ref_features, matched_cur_features, matched_cur_descriptors):
        if kVerbose:
            print('..................................')
        ...
        if(self.stage == VoStage.GOT_FIRST_IMAGE):
            self.processFrame(frame_id, matched_ref_features, matched_cur_features, matched_cur_descriptors)
    ...
    def drawFeatureTracks(self, img, reinit = False):
        ...
        if(self.stage == VoStage.GOT_FIRST_IMAGE):            
            if reinit:
                ...
                for i,pts in enumerate(zip(self.kps_ref, self.kpn_cur)):
