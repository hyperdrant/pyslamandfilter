...
class FeatureTrackingResult(object): 
    def __init__(self):
        ...
        self.des_ref_matched = None  # matched reference descriptors, des_ref_matched = des_ref[idxs_ref]
        self.des_cur_matched = None  # matched current descriptors, des_cur_matched = des_cur[idxs_cur]
...
class DescriptorFeatureTracker(FeatureTracker): 
    ...
    def track(self, image_ref, image_cur, kps_ref, des_ref):
        ...
        res.des_ref_matched = np.asarray(des_ref[idxs_ref]) # the matched ref descriptors
        res.idxs_ref = np.asarray(idxs_ref)                  
        
        res.kps_cur_matched = np.asarray(kps_cur[idxs_cur]) # the matched cur kps  
        res.des_cur_matched = np.asarray(des_cur[idxs_cur]) # the matched cur descriptors
